        ...
        <section id="autores" className="mb-32 scroll-mt-8 bg-background-light py-24 px-4 md:px-12 w-full flex flex-col items-center justify-center">
            ...
            <div className="max-w-[1200px] w-full flex flex-col md:flex-row items-stretch gap-6">
                {siteData.testimonials.map((testimonial, index) => (
                    <Reveal key={index} animation="reveal-scale" delay={index * 150} className="flex-1 flex">
                        <div
                            className="w-full flex flex-col items-start bg-white rounded-2xl shadow-sm border border-slate-100 p-8 text-left transition-transform hover:-translate-y-1 hover:shadow-md"
                        >
                            <div className="flex text-[#DD5E36] gap-1 mb-6">
                                {[...Array(testimonial.stars)].map((_, i) => (
                                    <Star key={i} className="text-[#DD5E36]" />
                                ))}
                            </div>
